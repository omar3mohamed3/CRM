...
import imageProperty from "/Logo.png";
...
const PropertyTable = ({ componentRef }) => {
  ...
  const { properties, loading, error, filters, pagination } = useSelector(
    (state) => state.properties
  );

  // Fetch Properties
  useEffect(() => {
    dispatch(fetchProperties({ ...filters, page: pagination.currentPage }));
  ...
  const handleSelectAll = () => {
    const allIds = properties.map((row) => row.id);
  ...
  // Delete Properties
  const handleDeleteProperties = (service) => {
    dispatch(deleteProperty(service.id))
      .unwrap()
      .then(() => toast.success(`Delete Lead ${service.title} Successfully`))
      .catch((error) => console.log(error));
  };

  const nextpage = () => {
    dispatch(setNextPropertyPage());
  };

  const previouspage = () => {
    dispatch(setPreviousPropertyPage());
  };
                  ...
                  {index === 0 ? (
                    <div className="  flex justify-left">
                      <input
                        ...
                        checked={
                          propertiesServices.length === properties.length
                        }
                      />
                    </div>
                  ) : (
            ...
            {loading ? (
              ...
              properties.map((row, index) => (
                <tr
                  ...
                  <td className="py-4 text-left">{row.title}</td>
                  <td className="py-4 text-center">{row.id}</td>
                  ...
                  {/* <td className="py-4 text-left">{row.assigned}</td> */}
                  <td className="py-4 text-left pr-1  max-w-[200px] text-gray-800">
                    {row.description.length > 20
                      ? row.description.slice(0, 20) + "..."
                      : row.description}
                  </td>
                  <td className="py-4 text-left">{row.type}</td>
                  ...
                  <td
                    ...
                    {showSetting === row.id && (
                      <div className="z-10 absolute -left-20 text-center bg-white rounded-[8px] shadow-card border py-1">
                        ...
                        <button
                          onClick={() => handleDeleteProperties(row)}
